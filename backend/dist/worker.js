...
async function runOnce() {
    ...
    try {
        ...
        // pick 1 job safely
        const jobPick = await client.query(`select id, report_id, user_id
             from jobs
             where status='queued'
        ...
        try {
            ...
            const rep = await client.query(`select file_key, filename from reports where id=$1 and user_id=$2`, [job.report_id, job.user_id]);
            if (!rep.rowCount)
            ...
            await client.query(`insert into analysis_results (report_id, user_id, result_json)
                 values ($1,$2,$3)
                 on conflict (report_id) do update set result_json=excluded.result_json, created_at=now()`, [job.report_id, job.user_id, analysis]);
...
async function main() {
    ...
    while (true) {
        try {
            ...
            console.error("[WORKER] main loop error", e?.message ?? e);
            await sleep(2000);
        }
        await sleep(2000);
    }
