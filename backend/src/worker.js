...
async function runOnce() {
    // pick 1 job safely
    const jobPick = await pool.query(`select id, report_id, user_id
     from jobs
     where status='queued'
    ...
    try {
        ...
        const rep = await pool.query(`select file_key, filename from reports where id=$1 and user_id=$2`, [job.report_id, job.user_id]);
        if (!rep.rowCount)
        ...
        await pool.query(`insert into analysis_results (report_id, user_id, result_json)
         values ($1,$2,$3)
